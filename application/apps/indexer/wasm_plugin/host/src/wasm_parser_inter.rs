...
const WASM_FILE_PATH: &str =
    "application/apps/indexer/wasm_plugin/client-inter/target/wasm32-wasi/release/client_inter.wasm";
...
#[allow(unused)]
pub struct WasmParserInter {
    engine: Engine,
    component: Component,
    linker: Linker<MyParserState>,
    store: Store<MyParserState>,
    parse_translate: ParseInter,
...
impl WasmParserInter {
    ...
    pub async fn create(
        ...
        let (parse_translate, _instance) =
            ParseInter::instantiate_async(&mut store, &component, &linker).await?;

        let empty_config = "";
        let parser_res = parse_translate
            .interface0
            .call_init(&mut store, empty_config)
            .await??;
        ...
        let raw_res = match self.cache.pop_front() {
            ...
            None | Some(Err(Error::Parse(_))) | Some(Err(Error::Incomplete)) => {
                let results = self
                    .parse_translate
                    .interface0
                    .call_parse(&mut self.store, input, timestamp)
                    .await
            ...
            let raw_res = match queue.pop_front() {
                ...
                None | Some(Err(Error::Parse(_))) | Some(Err(Error::Incomplete)) => {
                    self.parse_translate
                        .interface0
                        .call_parse_res(&mut self.store, input, timestamp)
                        .await
                        //TODO: Change this after implementing error definitions
                        .map_err(|err| {
                            println!("TODO AAZ: Early Error: {err}");
...
impl Parser<PluginParseMessage> for WasmParserInter {
    async fn parse<'a>(
        &mut self,
        input: &'a [u8],
        timestamp: Option<u64>,
        ...
        // match self.method {
        //     ParseMethod::ReturnVec => self.parse_with_list(input, timestamp).await,
        //     ParseMethod::ResSingle => self.parse_with_res(input, timestamp).await,
        //     ParseMethod::ResRange => self.parse_with_res_rng(input, timestamp).await,
        // }
    }
}
