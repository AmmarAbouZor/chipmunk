...
impl HostService {
    ...
    async fn open_single_file(&self, file_path: PathBuf) -> Result<(), HostError> {
        ...
        let format = file::detect_file_format(&file_path).map_err(InitSessionError::IO)?;
    ...
    async fn open_files_with_plugin(&self, paths: Vec<PathBuf>) -> Result<(), HostError> {
        let files = tokio::task::spawn_blocking(move || {
            paths
                .into_iter()
                .map(|path| {
                    file::detect_file_format(&path).map(|format| SourceFileInfo::new(path, format))
                })
    ...
    pub async fn open_multi_files(&self, paths: Vec<PathBuf>) -> Result<(), HostError> {
        let files: Vec<(PathBuf, FileFormat)> = tokio::task::spawn_blocking(move || {
            paths
                .into_iter()
                .filter_map(|path| {
